...
function Inventory({ bottles, setBottles }) {
  ...
  const [form, setForm] = useState({ name: "", category: "", level: 100 });
  ...
  const add = () => {
    ...
    setBottles((bs) => [
      ...bs,
      { id: "b" + Date.now(), name: form.name, category: form.category, level: Number(form.level), order: false },
    ]);
    setForm({ name: "", category: "", level: 100 });
    setAdding(false);
  };
  ...
  const visible =
    filter === "All" ? bottles : bottles.filter((b) => parentGroup(b.category) === filter);
